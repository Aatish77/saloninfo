<template>
  <v-card max-width="874" class="a">
    ...
    <v-card-text>
          ...
          <tr v-for="(item, index) in cart" :key="item.title">
            ...
            <v-dialog v-model="dialogOff" max-width="500px">
              <v-card>
                ...
                <v-card-text>
                  <book-appointment :cart="cart" :index="bookser" @child-value-updated="handleChildValueChange">{{ item.title }}</book-appointment>
                </v-card-text>
                <v-card-actions>
      ...
      <v-table v-else>
        <tbody>
          <tr v-for="(item, index) in cartFinal" :key="item.title">
            <td>{{ item.serTitle }}</td>
            <td><h6> {{ item.title }} </h6></td>
            <td>
              <v-btn class="btn1">
            ...
            <v-dialog v-model="dialogOff" max-width="500px">
              <v-card>
                <v-card-title> Book an Appointment </v-card-title>
                <v-card-text>
                  <book-appointment :cart="cartFinal ? cartFinal : cart" :index="bookser" @child-value-updated="handleChildValueChange">{{ item.title }}</book-appointment>
                </v-card-text>
                <v-card-actions>
                  <v-btn @click="toggleBookoff(null)">Close</v-btn>
                </v-card-actions>
  ...
  methods: {
   ...
   async reserve() {
      ...
      // You can now use this uniqueID to connect with the backend for Razorpay
       let res = await this.$store.dispatch("addpayMent", {'id':1,'amount':200})
       if(res){
